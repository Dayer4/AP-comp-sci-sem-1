...
class starter {
		...
		*/
		while(true){
			...
			if(choice.equals("change")){
				System.out.println("wat file do you want to change to");
				String newArr = sc.nextLine();	
				arr = fillArray(newArr);
			}
			//opt 6
			if(choice.equals("check")){
				System.out.print("wat word do you want to look for");
				String word = sc.nextLine();
				checksArray(arr,word);
			}
			//opt 7
	...
	*/
	public static void mostCommon(String[] arr){
		...
		System.out.println("the most common word is " + commonDupe);	// Dummy value, delete when needed.
	}
